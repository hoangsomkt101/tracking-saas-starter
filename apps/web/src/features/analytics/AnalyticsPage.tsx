      ...
      <section className="single-page-grid">
        ...
        <Card className="table-card">
          ...
          <CardContent>
            <div className="table-wrap">
              <table>
                <thead><tr><th>Timestamp</th><th>Matched click</th><th>Ref ID</th><th>Amount</th><th>Payout</th><th>Postback time</th><th>CAPI time</th><th>Delay</th></tr></thead>
                <tbody>
                  {ctx.tenantConversionEvents.map((event) => <tr key={event.id}><td>{event.postbackEventAt ? <>{formatDate(event.postbackEventAt)}<br /><small>{event.postbackEventDateField ?? 'timestamp'}: {event.postbackEventDateValue ?? event.postbackEventAt}</small></> : '—'}</td><td>{event.attribution?.matched ? <><Badge variant="success">{event.attribution.matchedByRefId ? 'Matched by Ref ID' : 'Matched'}</Badge><br /><small>{event.attribution.trackingLink?.slug ?? event.clickUuid ?? '—'}</small></> : <><Badge variant="muted">Unmatched</Badge><br /><small>{event.clickUuid ?? 'No click UUID'}</small></>}</td><td>{event.affiliateRefId ? <><Badge variant="secondary">{event.affiliateRefSource === 'impact_ref_click_id' ? 'Impact' : event.affiliateRefSource === 'partnerstack_customer_key' ? 'PartnerStack' : 'Ref'}</Badge><br /><button type="button" className="text-button" onClick={() => applyRefFilter(event.affiliateRefId!, event.affiliateRefSource)}><small>{event.affiliateRefId}</small></button></> : '—'}</td><td>{formatCurrencyAmount(event.postbackAmount ?? event.spendAmount, event.currency ?? 'USD')}</td><td>{formatCurrencyAmount(event.postbackPayout ?? event.payoutAmount ?? event.commissionAmount, event.currency ?? 'USD')}</td><td>{event.firstReceivedAt ? <>{formatDate(event.firstReceivedAt)}<br /><small>{event.firstReceivedField ?? 'postback time'}: {event.firstReceivedValue ?? event.firstReceivedAt}</small></> : formatDate(event.createdAt)}</td><td>{event.capiUpdatedAt ? <>{formatDate(event.capiUpdatedAt)}<br /><small>CAPI {event.capiStatus ?? 'updated'}</small></> : '—'}</td><td>{formatPostbackDelay(event.capiDelaySeconds)}</td></tr>)}
