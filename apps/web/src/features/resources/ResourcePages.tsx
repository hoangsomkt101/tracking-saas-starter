...
import { Building2, ExternalLink, Globe2, Layers3, Link2, Megaphone, Pencil, ShieldCheck, Trash2 } from 'lucide-react'
...
import { EventMappingBuilder, formatEventMapping, parseEventMappingInput } from '../affiliate-platforms/EventMappingBuilder'
        ...
        <Card className="form-card">
          ...
          <CardContent>
            ...
            <div className="table-wrap"><table>
              ...
              <tbody>{assignedTrackingLinks.map((link) => <tr key={link.id}><td><strong>{link.slug}</strong></td><td>{link.brand?.name ?? link.brandId}</td><td>{link.brand?.affiliatePlatform?.name ?? link.brand?.affiliatePlatformId ?? '—'}</td><td><a href={getTrackingLinkUrl(link)} target="_blank" rel="noreferrer">{getTrackingLinkPath(link)} <ExternalLink size={13} /></a></td><td><Button variant="outline" size="sm" type="button" onClick={() => void handleRemoveTrackingLink(link.id)}>Gỡ khỏi campaign</Button></td></tr>)}{!assignedTrackingLinks.length && <tr><td colSpan={5}>Campaign chưa có tracking link.</td></tr>}</tbody>
            </table></div>
          </CardContent>
        </Card>
      </div>
...
export function PlatformsPage({ ctx }: { ctx: DashboardContext }) {
  return (<><StatusBanner status={ctx.status} /><section className="resource-page"><PageToolbar title={<><Globe2 size={18} /> Affiliate platforms</>} description={`${ctx.tenantAffiliatePlatforms.length} affiliate networks. Quản lý ở các URL riêng.`} createPath="/platforms/new" createLabel="Thêm platform" /><Card className="table-card"><CardContent><div className="table-wrap"><table><thead><tr><th>Name</th><th>Slug</th><th>Tracking param</th><th>Webhook method</th><th>Default event</th><th>Rules</th><th>Actions</th></tr></thead><tbody>{ctx.tenantAffiliatePlatforms.map((platform) => <tr key={platform.id}><td><strong>{platform.name}</strong></td><td>{platform.slug}</td><td>{platform.trackingParamKey}</td><td>{platform.webhookMethod}</td><td>{platform.defaultEventName ?? 'CompleteRegistration'}</td><td>{platform.eventMapping?.length ?? 0}</td><td><ActionButtons detailPath={`/platforms/${platform.id}`} editPath={`/platforms/${platform.id}/edit`} deletePath={`/platforms/${platform.id}/delete`} /></td></tr>)}{!ctx.tenantAffiliatePlatforms.length && <tr><td colSpan={7}>Chưa có affiliate platform.</td></tr>}</tbody></table></div></CardContent></Card></section></>)
}
...
export function PlatformDetailPage({ ctx }: { ctx: DashboardContext }) { const { id = '' } = useParams(); const platform = ctx.tenantAffiliatePlatforms.find((item) => item.id === id); if (!platform) return <NotFoundEntity name="Affiliate platform" backPath="/platforms" />; return <EntityDetailCard title={<><Globe2 size={18} /> {platform.name}</>} description="Chi tiết affiliate platform." backPath="/platforms"><DetailGrid><DetailItem label="Platform ID" value={platform.id} /><DetailItem label="Webhook URL" value={`/affiliate-webhooks/${platform.tenantId}/${platform.slug}`} /><DetailItem label="Default event" value={platform.defaultEventName ?? 'CompleteRegistration'} /><DetailItem label="Rules" value={<pre>{formatEventMapping(platform.eventMapping)}</pre>} /><DetailItem label="Token" value={`${platform.webhookToken} (masked; use reveal/rotate API)`} /><DetailItem label="Created" value={formatDate(platform.createdAt)} /></DetailGrid></EntityDetailCard> }
export function PlatformEditPage({ ctx }: { ctx: DashboardContext }) { const { id = '' } = useParams(); const navigate = useNavigate(); const platform = ctx.tenantAffiliatePlatforms.find((item) => item.id === id); if (!platform) return <NotFoundEntity name="Affiliate platform" backPath="/platforms" />; const current = platform; async function handleSubmit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); await runEntityAction(ctx, async () => { await ctx.fetchJson<AffiliatePlatform>(`/affiliate-platforms/${current.id}`, { method: 'PUT', body: JSON.stringify({ name: getFormString(form, 'name'), slug: getFormString(form, 'slug'), trackingParamKey: getFormString(form, 'trackingParamKey'), webhookMethod: getFormString(form, 'webhookMethod'), defaultEventName: getFormString(form, 'defaultEventName'), eventMapping: parseEventMappingInput(getFormString(form, 'eventMapping')) }) }); navigate('/platforms') }, 'Đã cập nhật affiliate platform') } return <EntityDetailCard title={<><Pencil size={18} /> Sửa platform</>} description="Cập nhật platform ở trang riêng." backPath="/platforms"><form className="route-form" onSubmit={(event) => void handleSubmit(event)}><label><FieldLabel>Name</FieldLabel><Input name="name" defaultValue={current.name} required /></label><label><FieldLabel>Slug</FieldLabel><Input name="slug" defaultValue={current.slug} /></label><label><FieldLabel>Tracking param</FieldLabel><Input name="trackingParamKey" defaultValue={current.trackingParamKey} /></label><label><FieldLabel>Webhook method</FieldLabel><Select name="webhookMethod" defaultValue={current.webhookMethod}><option value="POST">POST</option><option value="GET">GET</option></Select></label><label><FieldLabel>Default event</FieldLabel><Input name="defaultEventName" defaultValue={current.defaultEventName ?? 'CompleteRegistration'} /></label><FieldLabel>Event mapping rules</FieldLabel><EventMappingBuilder name="eventMapping" defaultValue={current.eventMapping} ctx={ctx} tenantId={current.tenantId} defaultEventName={current.defaultEventName ?? 'CompleteRegistration'} /><Button type="submit">Lưu platform</Button></form></EntityDetailCard> }
  ...
  return (
    <EntityDetailCard title={<><Link2 size={18} /> {link.slug}</>} description="Chi tiết tracking link. Mỗi link có endpoint webhook riêng, tự map đúng brand/offer như luồng /r." backPath="/tracking-links">
      <DetailGrid>
        ...
        <DetailItem label="Shortlink" value={<a href={getTrackingLinkUrl(link)} target="_blank" rel="noreferrer">{getTrackingLinkUrl(link)}</a>} />
        <DetailItem label="Click webhook" value={<code>{getTrackingLinkWebhookUrl(link)}</code>} />
