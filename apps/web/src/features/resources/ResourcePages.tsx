...
function getAffiliateWebhookUrl(platform: AffiliatePlatform) {
  return `${apiBaseUrl}/affiliate-webhooks/${platform.tenantId}/${platform.slug}`
}

function getAffiliateWebhookPath(platform: AffiliatePlatform) {
  return `/affiliate-webhooks/${platform.tenantId}/${platform.slug}`
}
...
export function PlatformDetailPage({ ctx }: { ctx: DashboardContext }) { const { id = '' } = useParams(); const platform = ctx.tenantAffiliatePlatforms.find((item) => item.id === id); if (!platform) return <NotFoundEntity name="Affiliate platform" backPath="/platforms" />; return <EntityDetailCard title={<><Globe2 size={18} /> {platform.name}</>} description="Chi tiết affiliate platform." backPath="/platforms"><DetailGrid><DetailItem label="Platform ID" value={platform.id} /><DetailItem label="Platform" value={platform.platformLabel ?? platform.slug} /><DetailItem label="Webhook URL" value={<CopyableValue ctx={ctx} value={getAffiliateWebhookUrl(platform)} label="webhook URL"><code>{getAffiliateWebhookUrl(platform)}</code></CopyableValue>} /><DetailItem label="Webhook method" value={`${platform.webhookMethod} (GET/POST đều được nhận)`} /><DetailItem label="Token" value={`${platform.webhookToken} (masked; use reveal/rotate API)`} /><DetailItem label="Created" value={formatDate(platform.createdAt)} /></DetailGrid></EntityDetailCard> }
