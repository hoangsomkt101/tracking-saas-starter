...
import { ActionButtons, DetailGrid, DetailItem, EntityDetailCard, NotFoundEntity } from '../../components/common/EntityScaffold'
...
function getCampaignTrackingLinkCount(ctx: DashboardContext, campaign: Campaign) {
  return campaign.trackingLinks?.length ?? campaign._count?.trackingLinks ?? ctx.tenantTrackingLinks.filter((link) => link.campaignId === campaign.id).length
}
    ...
    <>
      ...
      <section className="resource-page">
        <Card className="table-card"><CardContent><div className="table-wrap"><table>
          ...
          <tbody>{ctx.tenantCampaigns.map((campaign) => <tr key={campaign.id}><td><strong>{campaign.name}</strong></td><td>{getCampaignTrackingLinkCount(ctx, campaign)}</td><td>{getCampaignDatasetSummary(campaign)}</td><td>{campaign.id}</td><td>{formatDate(campaign.createdAt)}</td><td><ActionButtons detailPath={`/campaigns/${campaign.id}`} editPath={`/campaigns/${campaign.id}/edit`} deletePath={`/campaigns/${campaign.id}/delete`} /></td></tr>)}{!ctx.tenantCampaigns.length && <tr><td colSpan={6}>Chưa có campaign.</td></tr>}</tbody>
...
export function CampaignDetailPage({ ctx }: { ctx: DashboardContext }) {
  ...
  const campaignQuery = useQuery({
    ...
    queryFn: () => ctx.fetchJson<Campaign>(`/campaigns/${id}`),
    enabled: Boolean(id),
    initialData: () => ctx.tenantCampaigns.find((item) => item.id === id)
  })
  const campaign = campaignQuery.data
  if (!campaign && !campaignQuery.isLoading) return <NotFoundEntity name="Campaign" backPath="/campaigns" />
  if (!campaign) return <EntityDetailCard title="Campaign" description="Đang tải dữ liệu campaign..." backPath="/campaigns"><p className="empty-state">Đang tải...</p></EntityDetailCard>
  ...
  return (
    <EntityDetailCard title={<><Megaphone size={18} /> {currentCampaign.name}</>} description="Quản lý campaign: chọn nhiều Dataset theo limit của gói tài khoản." backPath="/campaigns">
      <DetailGrid>
        ...
        <DetailItem label="Tracking links" value={`${assignedTrackingLinkCount} link`} />
              ...
              {ctx.tenantDatasets.map((dataset) => {
                ...
                return (
                  <label key={dataset.id} className={`feature-card-toggle dataset-card-toggle ${isSelected ? 'is-selected' : ''}`}>
                    ...
                    <Badge variant={isSelected ? 'success' : 'muted'}>{isSelected ? 'Đã chọn' : 'Chọn'}</Badge>
                  </label>
                )
              })}
            </div>
...
export function CampaignDeletePage({ ctx }: { ctx: DashboardContext }) {
  ...
  if (!campaign) return <NotFoundEntity name="Campaign" backPath="/campaigns" />
  const currentCampaign = campaign
  async function handleDelete() { await runEntityAction(ctx, async () => { await ctx.fetchJson<{ ok: boolean }>(`/campaigns/${currentCampaign.id}`, { method: 'DELETE' }); await ctx.refreshEntity('campaigns'); navigate('/campaigns') }, 'Đã xóa campaign') }
...
export function TrackingLinkDeletePage({ ctx }: { ctx: DashboardContext }) { const { id = '' } = useParams(); const navigate = useNavigate(); const link = ctx.tenantTrackingLinks.find((item) => item.id === id); if (!link) return <NotFoundEntity name="Tracking link" backPath="/tracking-links" />; const current = link; async function handleDelete() { await runEntityAction(ctx, async () => { await ctx.fetchJson<{ ok: boolean }>(`/tracking-links/${current.id}`, { method: 'DELETE' }); await ctx.refreshEntity('tracking-links'); navigate('/tracking-links') }, 'Đã xóa tracking link') } return <EntityDetailCard title={<><Trash2 size={18} /> Xóa tracking link</>} description="Xác nhận xóa ở URL riêng." backPath="/tracking-links"><div className="danger-zone"><p>Bạn sắp xóa shortlink <strong>{current.slug}</strong>. Click events liên quan có thể bị xóa theo.</p><Button variant="destructive" onClick={() => void handleDelete()}><Trash2 size={16} /> Xác nhận xóa</Button></div></EntityDetailCard> }
