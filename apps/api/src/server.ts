...
import { createClickEventsQueue, createFbc, createRedisConnection, getWebhookToken, maskSecret, normalizeAffiliateEventMapping, normalizeEventName, normalizeHeaderValue, parseEnvList, resolveAffiliateEventName, validateHttpUrl, type AffiliateEventMatch } from '@repo/shared'
...
function normalizeWebhookMethod(value: unknown) { return typeof value === 'string' && value.trim().toUpperCase() === 'GET' ? 'GET' : 'POST' }
...
function getDefaultTrackingParamKey(network: string) { const n = network.toLowerCase(); if (n.includes('partnerstack')) return 'sid1'; if (n.includes('firstpromo')) return 'ref_id'; return 'subid1' }
function normalizeTrackingParamKey(value: unknown, network: string) { return optionalString(value) ?? getDefaultTrackingParamKey(network) }
function getEventMappingInput(value: unknown) { return normalizeAffiliateEventMapping(value) as Prisma.InputJsonValue }
...
function serializeAffiliatePlatform<T extends { webhookToken?: string | null }>(platform: T) { return { ...platform, webhookToken: maskSecret(platform.webhookToken) } }
...
function resolvePlatformEventName(platform: { slug?: string | null; name?: string | null; eventMapping?: unknown; defaultEventName?: string | null }, payload: AnyRecord) { const impactMatch = getImpactEventMatch(payload); if (impactMatch) return impactMatch; return resolveAffiliateEventName(payload, platform.eventMapping, platform.defaultEventName) }
...
app.post('/affiliate-platforms', async (req, reply) => { const u = requireAuthenticated(req); const b = req.body as AnyRecord; const tenantId = requireString(b.tenantId, 'tenantId'); const name = requireString(b.name, 'name'); await assertTenantAccess(u.id, tenantId); const row = await prisma.affiliatePlatform.create({ data: { tenantId, name, slug: toSlug(optionalString(b.slug) ?? name), trackingParamKey: normalizeTrackingParamKey(b.trackingParamKey, name), webhookMethod: normalizeWebhookMethod(b.webhookMethod), defaultEventName: normalizeEventName(b.defaultEventName), eventMapping: getEventMappingInput(b.eventMapping) } }); await createActivityLog({ tenantId, source: 'api', eventType: 'affiliate_platform.created', message: `Affiliate platform "${row.name}" was created`, entityType: 'affiliatePlatform', entityId: row.id, metadata: { actorUserId: u.id, platformId: row.id, slug: row.slug, trackingParamKey: row.trackingParamKey, webhookMethod: row.webhookMethod } }); return reply.code(201).send(serializeAffiliatePlatform(row)) })
app.put('/affiliate-platforms/:id', async (req, reply) => { const u = requireAuthenticated(req); const { id } = req.params as { id: string }; const b = req.body as AnyRecord; const row = await prisma.affiliatePlatform.findFirst({ where: { id, tenant: { ownerUserId: u.id } } }); if (!row) return reply.code(404).send({ error: 'Affiliate platform not found' }); const name = optionalString(b.name) ?? row.name; const updated = await prisma.affiliatePlatform.update({ where: { id }, data: { name, slug: toSlug(optionalString(b.slug) ?? row.slug), trackingParamKey: normalizeTrackingParamKey(b.trackingParamKey ?? row.trackingParamKey, name), webhookMethod: normalizeWebhookMethod(b.webhookMethod ?? row.webhookMethod), defaultEventName: normalizeEventName(b.defaultEventName ?? row.defaultEventName), eventMapping: getEventMappingInput(b.eventMapping ?? row.eventMapping) } }); return serializeAffiliatePlatform(updated) })
...
app.post('/affiliate-platforms/test-event-mapping', async (req, reply) => {
  ...
  let platformForResolution: { slug?: string | null; name?: string | null; eventMapping?: unknown; defaultEventName?: string | null } = { eventMapping: mapping, defaultEventName }
...
app.route({
  ...
  handler: async (req, reply) => {
    ...
    const platform = await prisma.affiliatePlatform.findFirst({ where: { tenantId: p.tenantId, slug: p.platformSlug, webhookToken: requireWebhookToken(q.token, req.headers['x-webhook-token']), webhookMethod: method } })
