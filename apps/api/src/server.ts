...
function extractConversionMoney(payload: AnyRecord) { const isImpact = isImpactPostbackPayload(payload); return { spendAmount: getPayloadMoney(payload, isImpact ? ['Amount', 'amount', 'spendAmount', 'spend_amount', 'spend', 'cost', 'ad_spend'] : ['spendAmount', 'spend_amount', 'spend', 'cost', 'ad_spend']), payoutAmount: getPayloadMoney(payload, isImpact ? ['Payout', 'payout', 'payoutAmount', 'payout_amount', 'commissionAmount', 'commission_amount', 'commission'] : ['payoutAmount', 'payout_amount', 'payout', 'revenue', 'sale_amount', 'amount', 'value']), commissionAmount: getPayloadMoney(payload, ['commissionAmount', 'commission_amount', 'commission', 'profit']), currency: (getPayloadString(payload, ['currency', 'currencyCode', 'currency_code']) ?? 'USD').toUpperCase() } }
...
function getPostbackEventDate(payload: AnyRecord) { const keys = new Set(postbackEventDateKeys.map(normalizePayloadLookupKey)); for (const [field, value] of Object.entries(payload)) { if (!keys.has(normalizePayloadLookupKey(field)) || !isFilledPayloadValue(value)) continue; const raw = value instanceof Date ? value.toISOString() : String(value).trim(); const date = new Date(raw); if (!Number.isNaN(date.getTime())) return { field, raw, date } } return null }
...
function getPostbackAmount(payload: AnyRecord, e: AnyRecord) { return serializeMoneyValue(getPayloadMoney(payload, ['Amount', 'amount', 'saleAmount', 'sale_amount', 'orderAmount', 'order_amount', 'customerAmount', 'customer_amount', 'value']) ?? e.spendAmount) }
function getPostbackPayout(payload: AnyRecord, e: AnyRecord) { return serializeMoneyValue(getPayloadMoney(payload, ['Payout', 'payout', 'payoutAmount', 'payout_amount', 'commissionAmount', 'commission_amount', 'commission']) ?? e.payoutAmount ?? e.commissionAmount) }
function serializeConversion(e: AnyRecord, click?: AnyRecord) { const storedSnapshot = e.attributionSnapshot && typeof e.attributionSnapshot === 'object' ? e.attributionSnapshot as AnyRecord : null; const rawPayload = getPlainRecord(e.rawPayload) ?? {}; const postbackEventDate = getPostbackEventDate(rawPayload); return { ...e, id: e.id.toString(), clickEventId: e.clickEventId ? e.clickEventId.toString() : null, spendAmount: serializeMoneyValue(e.spendAmount), payoutAmount: serializeMoneyValue(e.payoutAmount), commissionAmount: serializeMoneyValue(e.commissionAmount), affiliatePlatform: e.affiliatePlatform ? serializeAffiliatePlatform(e.affiliatePlatform) : null, attribution: storedSnapshot ?? buildAttributionSnapshot(click, e.affiliatePlatform), capiEnrichment: e.capiEnrichment ?? null, postbackAmount: getPostbackAmount(rawPayload, e), postbackPayout: getPostbackPayout(rawPayload, e), postbackEventAt: postbackEventDate ? postbackEventDate.date.toISOString() : null, postbackEventDateField: postbackEventDate?.field ?? null, postbackEventDateValue: postbackEventDate?.raw ?? null, postbackDelaySeconds: postbackEventDate ? getPostbackDelaySeconds(e.createdAt, postbackEventDate.date) : null } }
