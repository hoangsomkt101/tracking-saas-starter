...
import { Prisma, activateTenantSubscription, approveWalletTopUp, billTenantSubscription, cancelWalletTopUp, createWalletTopUp, getWalletOverview, prisma, rejectWalletTopUp, type User } from '@repo/db'
...
function isPublicRoute(req: FastifyRequest) { const path = req.url.split('?')[0]; return path === '/health' || path === '/health/live' || path === '/health/ready' || path === '/metrics' || path === '/atp.js' || path === '/atp/events' || path === '/public/subscriptions' || path === '/payment-webhooks/sepay' || req.method === 'OPTIONS' || path.startsWith('/affiliate-webhooks/') }
  ...
  if (normalizedRequestedEventName === 'affiliateclick' || normalizedRequestedEventName === 'click') {
    ...
    try {
      ...
      if (clickEvent) {
        ...
        await assertSubscriptionLimit(tenant.id, 'clicks')
        try {
...
app.delete('/wallet/top-ups/:id', async (req, reply) => {
  ...
  const { id } = req.params as { id: string }
  try {
    const result = await cancelWalletTopUp(id, user.id)
    if (result.alreadyProcessed) return reply.code(400).send({ error: 'Only pending wallet top-ups can be cancelled' })
    return result.topUp
  } catch (error) {
    if (error instanceof Error && error.message === 'Wallet top-up not found') return reply.code(404).send({ error: error.message })
    throw error
  }
})
...
app.post('/superadmin/subscriptions', async (req, reply) => { requireSuperAdmin(req); const b = req.body as AnyRecord; const name = requireString(b.name, 'name'); const currency = (optionalString(b.currency) ?? 'VND').toUpperCase(); if (currency !== 'VND') throw new Error('Subscription currency must be VND'); const isDefault = optionalBoolean(b.isDefault, false); if (isDefault) await prisma.subscription.updateMany({ data: { isDefault: false } }); const subscription = await prisma.subscription.create({ data: { slug: toSlug(optionalString(b.slug) ?? name), name, description: optionalString(b.description), monthlyPriceCents: optionalInteger(b.monthlyPriceCents, 0), currency, clickLimit: optionalInteger(b.clickLimit, 1000), capiEventLimit: optionalInteger(b.capiEventLimit, 1000), eapiEventLimit: optionalInteger(b.eapiEventLimit, 1000), campaignDatasetLimit: optionalInteger(b.campaignDatasetLimit, 2), isDefault, isActive: optionalBoolean(b.isActive, true) } }); return reply.code(201).send(subscription) })
app.put('/superadmin/subscriptions/:id', async (req, reply) => { requireSuperAdmin(req); const { id } = req.params as { id: string }; const b = req.body as AnyRecord; const subscription = await prisma.subscription.findUnique({ where: { id } }); if (!subscription) return reply.code(404).send({ error: 'Subscription not found' }); const currency = (optionalString(b.currency) ?? subscription.currency).toUpperCase(); if (currency !== 'VND') throw new Error('Subscription currency must be VND'); const isDefault = optionalBoolean(b.isDefault, subscription.isDefault); if (isDefault) await prisma.subscription.updateMany({ where: { id: { not: id } }, data: { isDefault: false } }); const currentDatasetLimit = 'campaignDatasetLimit' in subscription ? Number(subscription.campaignDatasetLimit) : 2; return prisma.subscription.update({ where: { id }, data: { slug: b.slug ? toSlug(b.slug) : subscription.slug, name: optionalString(b.name) ?? subscription.name, description: typeof b.description === 'string' ? b.description : subscription.description, monthlyPriceCents: optionalInteger(b.monthlyPriceCents, subscription.monthlyPriceCents), currency, clickLimit: optionalInteger(b.clickLimit, subscription.clickLimit), capiEventLimit: optionalInteger(b.capiEventLimit, subscription.capiEventLimit), eapiEventLimit: optionalInteger(b.eapiEventLimit, subscription.eapiEventLimit), campaignDatasetLimit: optionalInteger(b.campaignDatasetLimit, currentDatasetLimit), isDefault, isActive: optionalBoolean(b.isActive, subscription.isActive) } }) })
app.delete('/superadmin/subscriptions/:id', async (req, reply) => { requireSuperAdmin(req); const { id } = req.params as { id: string }; const subscription = await prisma.subscription.findUnique({ where: { id }, select: { id: true, name: true, isDefault: true, _count: { select: { tenants: true, walletTransactions: true } } } }); if (!subscription) return reply.code(404).send({ error: 'Subscription not found' }); if (subscription.isDefault) return reply.code(400).send({ error: 'Subscription must not be the default subscription to be deleted' }); if (subscription._count.tenants > 0) return reply.code(400).send({ error: `Subscription must be unassigned before deletion; it is assigned to ${subscription._count.tenants} workspace(s)` }); if (subscription._count.walletTransactions > 0) return reply.code(400).send({ error: 'Subscription must not have billing transactions to be deleted' }); await prisma.subscription.delete({ where: { id } }); return { ok: true, id } })
app.get('/public/subscriptions', async () => prisma.subscription.findMany({ where: { isActive: true }, orderBy: [{ isDefault: 'desc' }, { monthlyPriceCents: 'asc' }, { createdAt: 'asc' }] }))
...
async function assertCampaignDatasetLimit(tenantId: string, desiredCount: number) { const subscription = await getTenantSubscriptionOrDefault(tenantId); const limit = subscription?.campaignDatasetLimit ?? 2; if (desiredCount > limit) throw new Error(`Dataset limit exceeded: ${desiredCount}/${limit} for subscription ${subscription?.name ?? 'current'}`); return limit }
...
app.route({
  ...
  handler: async (req, reply) => {
    ...
    if (existing) {
      ...
      await assertSubscriptionLimit(platform.tenantId, 'eapiEvents')
      try {
