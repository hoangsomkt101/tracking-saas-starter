...
function getImpactPayoutNumber(payload: AnyRecord) { return parseMoneyNumber(getPayloadValue(payload, ['Payout', 'payout'])) }
function getImpactActionTrackerEventName(payload: AnyRecord) { return normalizeEventName(getPayloadString(payload, ['ActionTrackerName', 'actionTrackerName', 'action_tracker_name']), '') || undefined }
function resolveImpactPostbackEventNames(platform: { slug?: string | null; name?: string | null; trackingParamKey?: string | null }, payload: AnyRecord, primaryEventName: string) {
  ...
  if (supported?.key === 'impact' && isImpactPostbackPayload(payload)) {
